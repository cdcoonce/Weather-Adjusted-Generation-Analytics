...
def _asset_color(asset_type: str) -> str:
    """Return the chart color for *asset_type*.

    Parameters
    ----------
    asset_type : str
        One of ``"Wind"``, ``"Solar"``, or any other string.
...
def _prep_generation_lines(
    ...
    for asset_id_val in df["asset_id"].unique(maintain_order=False).sort().to_list():
        ...
        # Take the first asset_type found (all rows for same asset share type).
        asset_type_val: str = (
            asset_rows["asset_type"][0] if asset_rows.shape[0] > 0 else ""
        )
        color = _asset_color(asset_type_val)
