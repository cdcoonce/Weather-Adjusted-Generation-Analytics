...
- ✅ CI is enabled via GitHub Actions:
  - Lint: Ruff (focused rules) on `src/` and `dags/`
...
- Jobs:
  - `lint`: `uv run ruff check src dags --select F,E9,I`
...
- Add unit tests for `src/loaders/dlt_pipeline.py` orchestration paths.

## Notes / risks

- Ruff is intentionally scoped to `src/` and `dags/` with a minimal ruleset to avoid blocking on repo-wide formatting/lint cleanup.
